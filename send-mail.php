<?php
/**
 * Handler du formulaire de contact biarritzburo.com
...
if (mb_strlen($besoin) > 80) {
    fail(400, 'Besoin invalide.');
}
...
try {
    ...
    $mail->setFrom($env['MAIL_FROM'], $env['MAIL_FROM_NAME'] ?? 'BiarritzBuro');
    $mail->addAddress($env['MAIL_TO'], $env['MAIL_TO_NAME'] ?? 'BiarritzBuro');
    $mail->addReplyTo($email, $nom);

    $prefix  = $env['MAIL_SUBJECT_PREFIX'] ?? '';
    $subject = $besoin !== '' ? $besoin : 'Demande de renseignements';
    $mail->Subject = $prefix . $subject . ' — ' . $nom;

    $bodyTxt  = "Nouvelle demande depuis biarritzburo.com\n\n";
    $bodyTxt .= "Nom & prénom : $nom\n";
    $bodyTxt .= "Email        : $email\n";
    $bodyTxt .= "Téléphone    : " . ($tel !== '' ? $tel : '(non renseigné)') . "\n";
    $bodyTxt .= "Besoin       : " . ($besoin !== '' ? $besoin : '(non renseigné)') . "\n";
    $bodyTxt .= "\n--- Message ---\n";
